...
interface Props {
  entryText: string
  analysis: AiAnalysis
  onConfirm: (finalAnalysis: AiAnalysis) => void   // ← 传回编辑后的版本
  onDiscard: () => void
}

export default function Analysis({ entryText, analysis, onConfirm, onDiscard }: Props) {
  const [editing, setEditing] = useState(false)
  const [edited, setEdited] = useState<AiAnalysis>(() => structuredClone(analysis))
      ...
      <div className="relative bg-stone-900 border-4 border-stone-600 w-full max-w-sm overflow-hidden"
        ...
        <div className="relative z-20 p-5 font-typewriter">
          <div className="text-[9px] text-amber-600/60 tracking-widest mb-4 flex justify-between">
            <span>ANALYSIS RESULT</span><span className="animate-pulse">●</span>
          </div>
          ...
          <div className="mb-4">
            <div className="text-[8px] text-amber-700/50 tracking-widest mb-2">STATUS DELTA</div>
            <div className="space-y-1">
              <D label="SAN" v={data.statusChanges.san ?? 0} editing={editing} onChange={(v) => upS('san', v)} />
              <D label="FOCUS" v={data.statusChanges.focus ?? 0} editing={editing} onChange={(v) => upS('focus', v)} />
              <D label="DRIVE" v={data.statusChanges.drive ?? 0} editing={editing} onChange={(v) => upS('drive', v)} />
            </div>
          </div>

          <div className="mb-5">
            <div className="text-[8px] text-amber-700/50 tracking-widest mb-2">SKILL EXPERIENCE</div>
            {data.skillXpChanges.length === 0 ? (
              <div className="text-[10px] text-amber-600/30 py-2">NO_CHANGES</div>
            ) : (
              <div className="space-y-1">
                {data.skillXpChanges.map((sc, i) => (
                  <SkillD key={i} change={sc} editing={editing} onChange={(v) => upSk(i, v)} />
                ))}
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            {editing ? (
              <>
                <button onClick={() => { setEditing(false); setEdited(structuredClone(analysis)) }}
              ...
              <>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setEditing(true)}
                    className="flex-1 py-2.5 border border-amber-800/30 text-amber-600/50 text-[10px] font-typewriter tracking-widest hover:border-amber-700/50 hover:text-amber-500/70">EDIT</button>
                  <button onClick={onDiscard}
                    className="flex-1 py-2.5 border border-amber-800/20 text-amber-700/30 text-[10px] font-typewriter tracking-widest hover:border-amber-700/40 hover:text-amber-600/50">DISCARD</button>
                </div>
              </>
            )}
          </div>
  ...
  function upS(key: keyof StatusAttributes, value: number) {
    setEdited((prev) => ({ ...prev, statusChanges: { ...prev.statusChanges, [key]: value } }))
  }
  function upSk(index: number, xpGain: number) {
    setEdited((prev) => {
...
function D({ label, v, editing, onChange }: { label: string; v: number; editing: boolean; onChange: (v: number) => void }) {
  const sign = v > 0 ? '+' : ''
  const c = v > 0 ? 'text-amber-400' : v < 0 ? 'text-amber-700/50' : 'text-amber-700/30'
  return (
    <div className="flex items-center justify-between py-1.5 px-2">
      <span className="text-[10px] text-amber-500/70 tracking-wider">{label}</span>
      {editing ? (
        <div className="flex items-center gap-1">
          <button onClick={() => onChange(v - 1)} className="w-5 h-5 border border-amber-800/40 text-amber-600/50 text-xs hover:border-amber-700/60">−</button>
        ...
        <span className={`text-[10px] tabular-nums ${c}`}>{v === 0 ? '·' : `${sign}${v}`}</span>
      )}
    </div>
  )
}

function SkillD({ change, editing, onChange }: { change: SkillXpChange; editing: boolean; onChange: (v: number) => void }) {
  return (
    <div className="flex items-center justify-between py-1.5 px-2">
      <div className="flex items-center gap-1.5">
        <span className="text-[10px] text-amber-500/70 tracking-wider">{change.skillName}</span>
        {change.isNewSkill && <span className="text-[7px] text-red-600/60 border border-red-700/30 px-1 uppercase tracking-widest">NEW</span>}
      </div>
      {editing ? (
        <div className="flex items-center gap-1">
          ...
          <input type="number" value={change.xpGain} onChange={(e) => onChange(Math.max(0, Number(e.target.value) || 0))}
            className="w-10 text-center bg-transparent border border-amber-800/40 py-0.5 text-[10px] text-amber-400 tabular-nums" />
          <button onClick={() => onChange(change.xpGain + 1)} className="w-5 h-5 border border-amber-800/40 text-amber-600/50 text-xs hover:border-amber-700/60">+</button>
        </div>
      ) : (
